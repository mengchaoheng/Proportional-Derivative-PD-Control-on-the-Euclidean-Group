...
- `xx_notation_corrections_and_supplements.tex`
   ...
   the old and new notation type-consistent, the removed wording is shown in red strikethrough.
...
```math
I_3+\frac{1}{2}\widehat{\psi}+(1-\alpha(\|\psi\|))\widehat{\psi}^{\,2}.
```

   The reconstruction uses

```math
I_3+\frac{1}{2}\widehat{\psi}+\frac{1-\alpha(\|\psi\|)}{\|\psi\|^2}\widehat{\psi}^{\,2}.
```
...
```math
\dot{x}=\omega_{\parallel}+\beta(\|x\|)\omega_{\perp}+\frac{1}{2}(\omega\times x).
```

   Reconstruction:

```math
\dot{x}=\omega_{\parallel}+\beta(\|x\|)\omega_{\perp}+\frac{1}{2}(x\times\omega).
```
...
```math
W=\frac{1}{2}\|g\|_{\mathrm{SO}(3)}^2+\frac{1}{2}\langle\omega^b,J\omega^b\rangle+\epsilon\langle\log(g),J\omega^b\rangle.
```

   Reconstruction:

```math
W=\frac{k_p}{2}\|g\|_{\mathrm{SO}(3)}^2+\frac{1}{2}\langle\omega^b,J\omega^b\rangle+\epsilon\langle\log(g),J\omega^b\rangle.
```
...
```math
U(g,V^b)=-f(g,V^s)-\begin{bmatrix}K_\omega\log_{\mathrm{SO}(3)}(R)\\K_vp\end{bmatrix}-K_dV^s.
```

   Reconstruction:

```math
U(g,V^s)=-f(g,V^s)-\begin{bmatrix}K_\omega\log_{\mathrm{SO}(3)}(R)\\K_vp\end{bmatrix}-K_dV^s.
```
...
```math
e_{\mathrm{hybrid}}=\begin{bmatrix}RR_d^T & p-p_d\\0 & 1\end{bmatrix}.
```

   Reconstruction:

```math
e_{\mathrm{hybrid}}=\begin{bmatrix}R_d^TR & p-p_d\\0 & 1\end{bmatrix}.
```
