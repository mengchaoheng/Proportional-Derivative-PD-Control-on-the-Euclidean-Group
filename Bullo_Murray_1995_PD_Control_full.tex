...
\ad_V=\begin{bmatrix}\widehat\omega&0\\\widehat v&\widehat\omega\end{bmatrix}.\tag{2.2}\label{eq:2.2}
\end{equation*}
...
\end{align*}
Note that, for small $X$, the matrix series in equation~\eqref{eq:2.14} is full rank and absolutely convergent, in particular at $X=0$ we have $\dot X=V^b=V^s$.

It is instructive now to sum the matrix series in equation~\eqref{eq:2.14} for the important cases of $\SO(3)$ and $\SE(3)$. In general, since the dimension of $G$ is finite, say $N$, the rank of the linear operator $\ad_X$ is also at most $N$ and by the Cayley--Hamilton theorem, there exist some functions $a_1(X),\ldots,a_N(X)$ such that
\begin{equation*}
...
\begin{equation*}
\calB_X=\begin{bmatrix}A(\psi)^{-T}&0\\ \star&A(\psi)^{-T}\end{bmatrix}.\tag{2.21}\label{eq:2.21}
\end{equation*}
...
\begin{equation*}
V^b=-\begin{bmatrix}k_\omega I_3&0\\0&(k_\omega+k_v)I_3\end{bmatrix}\log(g)\tag{4.2}\label{eq:4.2}
\end{equation*}
...
\begin{equation*}
U(g,V^b)=-f(g,V^b)-\begin{bmatrix}K_\omega\log_{\SO(3)}(R)\\R^TK_vp\end{bmatrix}-K_dV^b,\tag{4.6}\label{eq:4.6}
\end{equation*}
...
\begin{equation*}
U(g,V^b)=N(g,V^b)-\begin{bmatrix}k_\omega\log_{\SO(3)}(R)\\k_vR^Tp\end{bmatrix}-K_dV^b.\tag{4.13}\label{eq:4.13}
\end{equation*}
...
&=\id+\frac12\ad_X+A\ad_X^2+B\ad_X^4,\tag{A.2}\label{eq:A.2}
\end{align*}
where
\begin{align*}
...
\end{equation*}
Expression \eqref{eq:A.2} is now completely known. To prove equation~\eqref{eq:2.21}, we write out explicitly what $\ad_X^2$ and $\ad_X^4$ look like. From equation~\eqref{eq:2.2}
\begin{equation*}
...
\end{bmatrix}.
\end{align*}
Now it is enough to note that the $(1,1)$ and the $(2,2)$ blocks are equal and that the $(1,1)$ block is exactly the $\calB_{\widehat\psi}$ operator on $\SO(3)$. From Lemma~\ref{lem:so3-exp-coordinate-derivative} we have
\begin{equation*}
