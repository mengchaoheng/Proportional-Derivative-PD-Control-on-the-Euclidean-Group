...
\begin{revision}
\noindent\textbf{Editorial convention for this annotated version.}
...
\[
\Exp(x):=\exp(x^\wedge),\qquad \Log(g):=(\log g)^\vee,
\qquad
...
\end{equation*}
\begin{revision}
Here $\nu=V^\vee=[\omega^T\ v^T]^T$ uses the original rotation--translation ordering; only the notation distinguishing the abstract maps $\Ad_g,\ad_V$ from their matrix representations $\bAd_g,\bad_\nu$ is new.
\end{revision}
...
$
\begin{revision}
Here the original symbol $A(\theta)$ is also retained.  It is the translational factor in the
$\SE(2)$ exponential (the quantity denoted $V(\theta)$ in Sol\`a).  It plays the same structural
role as $A(\psi)$ above, while it is not the left Jacobian of $\SO(2)$ itself: $\SO(2)$ is
one-dimensional and abelian, hence $J_L^{\SO(2)}(\theta)=1$.
\end{revision}
...
\end{align*}
\begin{revision}
The two basis-free differentials in equations~\eqref{eq:2.9}--\eqref{eq:2.10} are denoted
\[
\JopR(X):=\int_0^1\Ad_{\exp(-\lambda X)}\,d\lambda,
\qquad
\JopL(X):=\int_0^1\Ad_{\exp(\lambda X)}\,d\lambda,
\]
so that
\[
V^b=\JopR(X)\dot X,\qquad V^s=\JopL(X)\dot X.
\]
After choosing a basis, their matrix representations are
\[
\JR(x):=\vee\circ\JopR(x^\wedge)\circ\wedge,
\qquad
\JL(x):=\vee\circ\JopL(x^\wedge)\circ\wedge.
\]
Thus, with $v^b=(V^b)^\vee$ and $v^s=(V^s)^\vee$,
\[
\dot x=\JR^{-1}(x)v^b=\JL^{-1}(x)v^s.
\]
\end{revision}
...
\del{In the following we will sometime write equation~\eqref{eq:2.14} by introducing the auxiliary operator symbol $\mathcal B_X$.}
\begin{revision}
For traceability, the original auxiliary notation was
\begin{center}
\sout{\(\dot X=\mathcal B_XV^b\)}\hspace{2em}
\sout{\(\mathcal B_X=\displaystyle\sum_{n=0}^{\infty}\frac{(-1)^nB_n}{n!}\ad_X^n\)}.
\end{center}
\end{revision}
\begin{revision}
In this annotated edition the auxiliary symbol used in the original manuscript for
the inverse differential is suppressed. Equation~\eqref{eq:2.14} is written simply as
\[
\dot X=\JopR^{-1}(X)V^b,\qquad
\JopR^{-1}(X)
=\sum_{n=0}^{\infty}\frac{(-1)^nB_n}{n!}\ad_X^n,
\]
or, after choosing a basis,
\[
\dot x=\JR^{-1}(x)v^b,\qquad
\JR^{-1}(x)
=\sum_{n=0}^{\infty}\frac{(-1)^nB_n}{n!}\bad_x^n.
\]
The spatial counterpart is
\[
\dot X=\JopL^{-1}(X)V^s,\qquad
\dot x=\JL^{-1}(x)v^s.
\]
\end{revision}
...
\begin{revision}
Using the identification noted above, the same statement is simply
\[
\dot\psi=J_R^{\SO(3)}(\psi)^{-1}\omega,
\]
while the original $A(\psi)^{-T}$ notation is retained in the manuscript formulas.
\end{revision}
...
\end{align*}
\begin{revision}
To avoid overloading the already defined matrix function $A(\psi)$, the scalar functions denoted $A(y),B(y)$ in the original manuscript are written here as $\gamma_2(y),\gamma_4(y)$.
\end{revision}
where
\begin{align*}
...
\end{equation*}
\begin{revision}
This is the same $\SE(3)$ Jacobian coupling that appears as the $Q_R$ block in modern references.  With the present rotation--translation ordering one may write
\[
J_R^{\SE(3)}(x)=\begin{bmatrix}J_R^{\SO(3)}(\psi)&0\\Q_R(q,\psi)&J_R^{\SO(3)}(\psi)\end{bmatrix},
\]
...
\end{equation*}
\begin{revision}
Equation~\eqref{eq:4.2} is one of the places where the original manuscript implicitly identifies $\se(3)$ with $\mathbb R^6$: the displayed block gain acts on coordinates. The annotated equation therefore uses $\Log(g)$ explicitly while retaining the manuscript's rotation--translation ordering $\nu=[\omega^T\ v^T]^T$.
\end{revision}
...
\end{align*}
\begin{revision}
The scalar coefficients denoted $A,B$ in the original appendix are written as $\gamma_2,\gamma_4$ here, matching the renaming in Lemma~\ref{lem:se3-exp-coordinate-derivative} and avoiding collision with the matrix function $A(\psi)$.
\end{revision}
where
\begin{align*}
