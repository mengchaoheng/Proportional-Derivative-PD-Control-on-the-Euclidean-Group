...
\begin{revision}
Thus, $\JopR^{-1}(X):\gfrak\to\gfrak$ is the inverse right-Jacobian
operator associated with the exponential map.  After choosing a basis,
with
\[
x=X^\vee,\qquad v^b=(V^b)^\vee,
\]
...
=
\sum_{n=0}^{\infty}
\frac{(-1)^nB_n}{n!}\bad_x^n.
\]
Equivalently,
\[
\JR^{-1}(x)
=
\vee\circ\JopR^{-1}(X)\circ\wedge.
\]

For the spatial velocity, equation~\eqref{eq:2.15} gives the corresponding
left-Jacobian relations
\[
\dot X=\JopL^{-1}(X)V^s,
\qquad
\dot x=\JL^{-1}(x)v^s,
