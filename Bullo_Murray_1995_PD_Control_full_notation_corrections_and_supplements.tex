...
\end{equation*}
\begin{revision}
The translational factor in the $\SE(3)$ exponential coincides with the $\SO(3)$ left Jacobian:
\[
A(\psi)=J_l^{\SO(3)}(\psi).
\]
Since $J_r^{\SO(3)}(\psi)=J_l^{\SO(3)}(\psi)^T$, it follows that
\[
A(\psi)^{-T}=J_r^{\SO(3)}(\psi)^{-1}.
\]
Sol\`a\redstarfootnote{%
See Sol\`a et al., \emph{A micro Lie theory for state estimation in robotics},
Appendices B and D, Eqs.~(143)--(146) and (174).} denotes the same factor by $V(\psi)$.
\end{revision}
...
\]
Accordingly, $\log$ is used for Lie-algebra-valued expressions, whereas $\Log$ is used for their coordinate-vector representations.
Thus, with $X=x^\wedge$,
\[
g=\exp(X)=\Exp(x),\qquad
...
\end{align*}
and $\alpha(y)=(y/2)\cot(y/2)$, $\beta(y)\triangleq(y/2)^2/\sin^2(y/2)$. \del{Additionally the operator $\mathcal B_X$ can be written as} \rev{Additionally, for $x=X^\vee=[\psi^T\ q^T]^T$, the matrix representation of $\JopR^{-1}(X)$ is}
\begin{equation*}
...
\begin{equation*}
W=\frac{k_p}{2}\|g\|_{\SO(3)}^2+\frac12\langle\omega^b,\rev{\mathbb I}\omega^b\rangle_{\R^3}+\eps\langle\rev{\Log(g)},\rev{\mathbb I}\omega^b\rangle_{\rev{\R^3}},
\end{equation*}
...
\end{equation*}
Expression \eqref{eq:A.2} is now completely known. \del{To prove equation~\eqref{eq:2.21}, we write out explicitly what $\ad_X^2$ and $\ad_X^4$ look like.} \rev{To prove equation~\eqref{eq:2.21}, let $x=X^\vee=[\psi^T\ q^T]^T$ and write the corresponding matrices $\bad_x^2$ and $\bad_x^4$.} From equation~\eqref{eq:2.2}
\begin{equation*}
...
\end{bmatrix}}.
\end{align*}
\del{Now it is enough to note that the $(1,1)$ and the $(2,2)$ blocks are equal and that the $(1,1)$ block is exactly the $\mathcal B_{\widehat\psi}$ operator on $\SO(3)$.} \rev{The two diagonal blocks are the same $\SO(3)$ block appearing in Lemma~\ref{lem:so3-exp-coordinate-derivative}, hence each is $A(\psi)^{-T}$. Using $A(\psi)^{-T}=J_r^{\SO(3)}(\psi)^{-1}$, equation~\eqref{eq:2.21} becomes}
\begin{equation*}
\rev{\JR^{-1}(x)=J_r^{\SE(3)}(x)^{-1}=\begin{bmatrix}A(\psi)^{-T}&0\\ *&A(\psi)^{-T}\end{bmatrix}
=\begin{bmatrix}J_r^{\SO(3)}(\psi)^{-1}&0\\ *&J_r^{\SO(3)}(\psi)^{-1}\end{bmatrix}}.
\end{equation*}
\proofbox
