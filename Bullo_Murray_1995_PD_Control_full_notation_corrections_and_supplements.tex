...
\end{equation*}
\begin{revision}
Choose a basis $\mathcal E=(E_1,\ldots,E_n)$ of $\gfrak$ and let
$X=x^\wedge$, $Y=y^\wedge$.  The coordinate matrices of the two maps above are
characterized by
\[
\bAd_g y
:=\bigl(\Ad_g(Y)\bigr)^\vee
=\bigl(g\,y^\wedge g^{-1}\bigr)^\vee,
\]
\[
\bad_x y
:=\bigl(\ad_X(Y)\bigr)^\vee
=\bigl[x^\wedge,y^\wedge\bigr]^\vee
=\bigl(x^\wedge y^\wedge-y^\wedge x^\wedge\bigr)^\vee.
\]
Equivalently,
\[
...
\[
(\ad_VY)^\vee=\bad_\nu y
=\begin{bmatrix}\widehat\omega\eta\\
\widehat v\eta+\widehat\omega u\end{bmatrix}.
\]
\end{revision}
