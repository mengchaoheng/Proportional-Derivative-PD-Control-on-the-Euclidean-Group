...
\end{equation*}
\begin{revision}
Choose a basis $\mathcal E=(E_1,\ldots,E_n)$ of $\gfrak$ and write
\[
X=x^\wedge,\qquad Y=y^\wedge,
\qquad x=X^\vee,\qquad y=Y^\vee.
\]
For any linear map $T:\gfrak\to\gfrak$, its matrix in these coordinates is the
unique matrix $\mathbf T\in\R^{n\times n}$ satisfying
\[
\bigl(T(Y)\bigr)^\vee=\mathbf T\,y
\qquad\text{for all }Y\in\gfrak.
\]
Since $Y=y^\wedge$, this is equivalently
\[
\mathbf T\,y=\bigl(T(y^\wedge)\bigr)^\vee,
\qquad\text{hence}\qquad
\mathbf T=\vee\circ T\circ\wedge.
\]
Applying this to the two maps above gives
\[
...
On $\SE(3)$ and $\se(3)$ we represent a group element
$g=(R,p)\in\SO(3)\times\R^3$ and a velocity
$V=(\widehat\omega,v)\in\so(3)\times\R^3$ using homogeneous coordinates,
\begin{equation*}
g=
\begin{bmatrix}
R&p\\
...
\end{bmatrix},
\end{equation*}
where the operator $\widehat{\phantom{x}}:\R^3\to\so(3)$ is defined so
that $\widehat{x}y=x\times y$ for all $x,y\in\R^3$.
...
y=Y^\vee=
\begin{bmatrix}\eta\\u\end{bmatrix},
\qquad
\nu=V^\vee=
\begin{bmatrix}\omega\\v\end{bmatrix}.
\]
The abstract maps give
\[
\Ad_g(Y)=gYg^{-1}
=
\begin{bmatrix}
\widehat{R\eta} & \widehat pR\eta+Ru\\
0&0
\end{bmatrix},
\]
and
...
=
\begin{bmatrix}
\widehat{\omega\times\eta} & \widehat v\eta+\widehat\omega u\\
0&0
\end{bmatrix}.
\]
Taking coordinates therefore yields
\[
\bigl(\Ad_g(Y)\bigr)^\vee
=
\begin{bmatrix}R\eta\\ \widehat pR\eta+Ru\end{bmatrix}
=
\bAd_g
\begin{bmatrix}\eta\\u\end{bmatrix},
\]
...
=
\begin{bmatrix}\widehat\omega\eta\\ \widehat v\eta+\widehat\omega u\end{bmatrix}
=
\bad_\nu
\begin{bmatrix}\eta\\u\end{bmatrix}.
\]
Hence the corresponding $6\times6$ matrices are
\end{revision}
...
\]
Sol\`a denotes the same factor by $V(\psi)$.
\end{revision}
...
\end{align*}
\begin{revision}
Define the right- and left-trivialized differentials of the exponential by
\[
...
V^s=\JopL(X)\dot X.
\]
For $x=X^\vee$ and $u\in\R^n$, their coordinate matrices are defined by
\[
\JR(x)u:=\bigl(\JopR(X)(u^\wedge)\bigr)^\vee,
\qquad
\JL(x)u:=\bigl(\JopL(X)(u^\wedge)\bigr)^\vee,
\]
or equivalently
\[
...
\begin{revision}
Applying $\vee$ to the inverse differential gives
\[
\dot x=\JR^{-1}(x)\nu^b,
\qquad
\JR^{-1}(x)
...
\nu^b=(V^b)^\vee.
\]
Equivalently, for $u\in\R^n$,
\[
\JR^{-1}(x)u
=\bigl(\JopR^{-1}(x^\wedge)(u^\wedge)\bigr)^\vee.
\]
