...
\begin{revision}
The translational Lie-algebra coordinate is written as $q$ to distinguish it from
the group translation $p$.
\end{revision}
\begin{align*}
...
\begin{revision}
The two series in Theorem~\ref{thm:differential-exponential} are the basis-free inverse right and left differentials of the exponential map:
\begin{align*}
\JopR^{-1}(X)
...
\end{align*}
After choosing a basis, with $x=X^\vee$, their matrix representations are
\begin{align*}
\JR^{-1}(x)&=\sum_{n=0}^{\infty}\frac{(-1)^nB_n}{n!}\bad_x^n,\\
\JL^{-1}(x)&=\sum_{n=0}^{\infty}\frac{B_n}{n!}\bad_x^n.
\end{align*}
In the terminology used by Sol\`a et al.\redstarfootnote{%
See Sol\`a et al., \emph{A micro Lie theory for state estimation in robotics},
Sec.~III-B3, Eqs.~(67)--(74).}, these are the inverse right and inverse left Jacobians of the exponential map; their inverses are denoted $J_r(x)$ and $J_l(x)$, respectively. Thus the theorem is equivalently
\[
\dot x=\JR^{-1}(x)\nu^b=\JL^{-1}(x)\nu^s,
\qquad
\nu^{b,s}:=(V^{b,s})^\vee.
...
\begin{revision}
With $x=\Log(R)$, Lemma~\ref{lem:so3-exponential-coordinates} is
\[
...
\]
Thus the trace is taken over the linear operator $\ad_X\circ\ad_Y:\gfrak\to\gfrak$.
After choosing a basis, with $x=X^\vee$ and $y=Y^\vee$, the corresponding matrix expression is $\langle X,Y\rangle_K = \tr(\bad_x\,\bad_y)$, where the product is ordinary matrix multiplication.
\end{revision}
...
+\epsilon\langle\rev{\Log(g)},\rev{\mathbb I}\omega^b\rangle_{\rev{\R^3}}
\end{equation*}
